...
class DifferentialInvariantsTupletsDataset(DeepSignatureTupletsDataset):
    ...
    # def __getitem__(self, index):
    #     new_tuplet = super().__getitem__(index=index)
    #     k = numpy.zeros(len(self._tuplets))
    #     ks = numpy.zeros(len(self._tuplets))
    #     for i, tuplet in enumerate(self._tuplets):
    #         anchor = tuplet[0]
    #         sample_k = curve_processing.calculate_euclidean_curvature(curve=anchor)
    #         sample_ks = curve_processing.calculate_euclidean_ks(curve=anchor)
    #         k[i] = sample_k[sample_k.shape[0] // 2]
    #         ks[i] = sample_ks[sample_ks.shape[0] // 2]
    #
    #     cov = numpy.abs(numpy.mean((k - k.mean()) * (ks - ks.mean())))
    #     corr = numpy.corrcoef(x=k, y=ks)
    #     return new_tuplet
