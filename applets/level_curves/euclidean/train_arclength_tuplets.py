...
if __name__ == '__main__':
    ...
    dataset.load_dataset(dir_path=settings.level_curves_euclidean_arclength_tuplets_dir_path)
    model = DeepSignatureArcLengthNet(sample_points=40).cuda()
    print(model)

    device = torch.device('cuda')
    all_subdirs = all_subdirs_of(settings.level_curves_euclidean_arclength_tuplets_results_dir_path)
