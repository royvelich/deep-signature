...
def calculate_signature_curve(curve, transform_type, sampling_ratio, models, rng=None, plot=False, transform_curve=True):
    ...
    signature_curve = predicted_curve_invariants['predicted_differential_invariants']
...
def evaluate_raw_curves_signatures_worker(queue, models, raw_curves, transform_type, dataset_name):
...
def compare_signatures_worker(queue, models, raw_curves_signatures, downsampled_curves, sampling_ratio, transform_type, dataset_name):
    ...
    for i, curve in enumerate(downsampled_curves):
        ...
        for j, signature_curve in enumerate(raw_curves_signatures):
            ...
            distances[i, j] = evaluation_utils.calculate_hausdorff_distance(curve1=anchor_signature_curve, curve2=signature_curve, distance_type='euclidean')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--results-base-dir-path", type=str)
    ...
    for dataset_name in dataset_names:
        ...
        for i, _ in enumerate(raw_curves):
            raw_curves[i] = raw_curves[i].astype('float32')
        p = torch_mp.Process(target=evaluate_raw_curves_signatures_worker, args=(raw_curves_queue, models, raw_curves, transform_type, dataset_name,))
        processes.append(p)
        p.start()
    ...
    for raw_curves_config in raw_curves_configs:
        ...
        for sampling_ratio in sampling_ratios:
            ...
            for i, _ in enumerate(downsampled_curves):
                downsampled_curves[i] = downsampled_curves[i].astype('float32')
            p = torch_mp.Process(target=compare_signatures_worker, args=(signature_comparison_queue, models, signatures, downsampled_curves, sampling_ratio, transform_type, dataset_name,))
            processes.append(p)
            p.start()
